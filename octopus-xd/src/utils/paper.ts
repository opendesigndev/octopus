...
export function createRectangleFromPointSize(
  point: paper.Point,
  size: paper.Size
): paper.Rectangle {
...
export function createSegmentI(
  point: paper.Point,
  handleIn: paper.Point
): paper.Segment {
...
export function createPath(
  options: Record<string, unknown> | paper.Segment[] | string
): paper.Path {
  return new Path(options)
}
export function createPathRectangle(
  point: paper.Point,
  size: paper.Size
): paper.Path.Rectangle {
  return new Path.Rectangle(point, size)
}
export function createPathEllipse(
  point: paper.Point,
  size: paper.Size
): paper.Path.Ellipse {
...
export function createCompoundPath(
  ...
  return new CompoundPath(
    typeof options === 'string' ? { pathData: options } : options
  )
}
