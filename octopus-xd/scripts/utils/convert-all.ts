...
export async function convertAll(options: ConvertAllOptions): Promise<string[]> {
  const id = uuidv4()
  ...
  const convertedManifest = await saver(`octopus-manifest.json`, stringify(convertedDesign.manifest))
  ...
  return convertedDesign.artboards.reduce(async (queue, conversion) => {
    ...
    const sourceLocation = await saver(`source-${conversion.targetArtboardId}.json`, stringify(artboard?.raw))
    const octopusLocation = await saver(`octopus-${conversion.targetArtboardId}.json`, stringify(conversion.value))
    ...
    const render =
      options.render && !conversion.error ? await renderOctopus(conversion.targetArtboardId, octopusLocation) : null
