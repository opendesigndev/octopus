...
export default class OctopusLayerShapeShapeAdapter extends OctopusLayerCommon implements OctopusLayerShapeAdapter {
  ...
  private _createClippingPaths(): Octopus['PathLike'][] {
    return asArray(
      this._sourceLayer.clippingPaths
        ?.map((sourceLayer) => {
          return { ...new OctopusLayerShapeShapeAdapter({ parent: this, sourceLayers: [sourceLayer] })._getPath() }
        })
