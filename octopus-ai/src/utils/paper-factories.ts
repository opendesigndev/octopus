...
export function createRectangleFromBounds(bounds: OctopusBounds): paper.Rectangle {
  return createRectangleFromPointSize(createPoint(bounds.left, bounds.top), createSize(bounds.width, bounds.height))
}
...
export function createCompoundPath(pathData: string | Record<string, unknown>): paper.CompoundPath {
