...
describe('entities/octopus/octopus-layer-mask-group', () => {
  ...
  describe('_isMaskValid', () => {
    ...
    it('returns true when subpaths are not only rects', () => {
      const maskGroup = getTestingMaskGroup()
      const mask: any = { shape: { path: { type: 'COMPOUND' } } }
      const _doesPathConsistsOnlyFromRectSubpathsMock = jest.fn().mockReturnValueOnce(false)

      maskGroup['_doesPathConsistsOnlyFromRectSubpaths'] = _doesPathConsistsOnlyFromRectSubpathsMock
      expect(maskGroup['_isMaskValid'](mask)).toEqual(true)
    })
