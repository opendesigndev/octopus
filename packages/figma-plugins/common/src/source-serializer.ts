...
async function nodeToObject(node: any, isTopNode = false) {
  ...
  try {
    ...
    if (OPTIONS.exportPreviews && isTopNode) await setPreview(node)
...
export async function getSourceData({ version, exportPreviews = false }: SourceDataOptions) {
  imageMap = {} // clear imageMap
  previewMap = {} // clear previewMap
  OPTIONS.exportPreviews = exportPreviews
