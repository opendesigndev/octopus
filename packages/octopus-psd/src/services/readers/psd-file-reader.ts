...
export type PSDFileReaderOptions = {
  ...
  /**File read to buffer */
  promiseBuffer: Promise<Buffer | ArrayBuffer>
...
export abstract class PSDFileReader {
  private _promiseBuffer: Promise<Buffer | ArrayBuffer>
  ...
  constructor(options: PSDFileReaderOptions) {
    this._promiseBuffer = options.promiseBuffer
  ...
  private async _getPsd(): Promise<Psd> {
    const resolvedBuffer = await this._promiseBuffer
    const buffer = 'buffer' in resolvedBuffer ? resolvedBuffer.buffer : resolvedBuffer
